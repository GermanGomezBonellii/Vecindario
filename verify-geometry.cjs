...
for (const levelNumber of [1,3,5,8,12,20]) {
  ...
  for (let i=0;i<12;i++) {
    ...
    for (const house of map.houses) {
      ...
      assert.ok(Math.abs(house.rect.width-house.rect.height)<1e-6);
      ...
      const expected = map.roadSegments.filter(s => s.enabled && (s.orientation==='H'
        ? (Math.abs(s.y1-ly)<1e-6 || Math.abs(s.y1-ly-map.cellSize)<1e-6) && Math.min(s.x2,lx+map.cellSize)-Math.max(s.x1,lx)>1e-6
        : (Math.abs(s.x1-lx)<1e-6 || Math.abs(s.x1-lx-map.cellSize)<1e-6) && Math.min(s.y2,ly+map.cellSize)-Math.max(s.y1,ly)>1e-6)).map(s=>s.streetKey);
