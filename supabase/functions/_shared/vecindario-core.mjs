/* Generated by node build.cjs — no editar a mano */
// ---- config.js ----
...
function getAdvancedTier(level) {
  return PROGRESSION.advanced.filter(t=>level>=t.from).at(-1) || null;
}
function getMaxGridWidth(level) {
...
function generateMap(rng, { cols = 4, rows = 3, houseCount = 8, streetBreaks = 0, missingBlocks = 0, levelNumber = 1, maxGridWidth = null, maxGridHeight = null } = {}) {
...
function calculateDifficulty(level, minimum = findMinimumSolvingSubsets(level)) {
...
export { CONFIG, DAILY_VERSIONS, DAILY_EPOCH, dailyDateKey, dailyVersionFor, dailySeed, generateDailyLevel, dailyFingerprint, replayDaily, gradeDaily, shiftDateKey, isDateKey, msUntilNextDaily };
